...
class OpenAIProvider {
  ...
  async transcribeAudio(audioPath) {
    try {
      ...
      const response = await this.client.audio.transcriptions.create({
        ...
        file: audioFile,
        language: 'es', // Forzar español, o quitar para autodetección
  ...
  _handleError(method, error) {
    ...
    if (error.status === 429) {
      throw new Error('Límite de tasa de OpenAI excedido. Intente más tarde.');
    }

    if (error.status === 500) {
      throw new Error('Error interno de OpenAI. Intente más tarde.');
    }
